...
class Scheduler:
    ...
    def __init__(self) -> None:
        ...
        self._cancelled_ids: set[EventId] = set()
    ...
    def pending_events(self) -> int:
        return sum(1 for item in self._heap if item[3] not in self._cancelled_ids)
    ...
    def cancel(self, event_id: EventId) -> bool:
        if event_id in self._cancelled_ids or not self._is_pending(event_id):
            return False
    ...
    def step(self) -> tuple[StepOutcome, DispatchedEvent | None]:
        while self._heap:
            ...
            if event_id in self._cancelled_ids:
                self._cancelled_ids.remove(event_id)
                continue
