...
| Contributor cannot run `just` recipes because `just` is missing from `PATH` | 3 | 3 | 9 | Keep underlying npm/node commands documented, include `just --version` in `just dev-validate`, and provide `scripts/bootstrap.ps1` to install or check the Windows path. | dx-agent | `just --list`, `just docs-bootstrap`, or `just check-docs` is not recognized after running the relevant bootstrap path |
