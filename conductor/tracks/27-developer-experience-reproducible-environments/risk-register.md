...
| Contributor cannot run `just` recipes because `just` is missing from `PATH` | 3 | 3 | 9 | Keep underlying npm/node commands documented and include `just --version` in `just dev-validate`; current shell evidence shows recipe execution is blocked until `just` is installed or added to `PATH` | dx-agent | `just --list`, `just docs-bootstrap`, or `just check-docs` is not recognized on a supported developer environment |
