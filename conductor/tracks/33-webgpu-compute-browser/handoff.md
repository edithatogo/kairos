...
- The demo and track matrix now record fallback-boundary status as complete; host crate runtime tests pass under the GNU toolchain, while browser WebGPU execution and cross-browser checks remain blocked until Wasm/browser wiring and GPU-capable validation are available.
...
- Optional host runtime gate: `pwsh -NoProfile -File conductor/tracks/33-webgpu-compute-browser/validate-track33.ps1 -RunRuntimeTests` now passes by running crate tests through `stable-x86_64-pc-windows-gnu`; this proves the host fallback and contract tests, not browser WebGPU device execution. Browser/WebGPU runtime proof remains blocked until Wasm binding and headless Chrome WebGPU validation are available.
...
- **webgpu-framerate** — staged future target for sustained frame-rate evidence on reference browser hardware after real WebGPU dispatch is configured. Informational only; becomes blocking at RC.
...
- Low-cost smoke routes are documented in `docs/gpu-compute/free-testing-routes.md`: use GitHub-hosted macOS runners or the M1 MacBook Pro for Metal-adjacent browser/device smoke, Colab GPU for the available T4-style runtime, and Colab TPU only for future TPU-specific prototypes. These routes do not satisfy browser WebGPU parity or frame-rate evidence.
...
- The 100K-agent frame-rate claim is blocked until a real WebGPU backend and reference hardware/browser results exist.
...
- Accepted fix: `BufferBridge` now rejects partial elements where `len_bytes` is smaller than or not divisible by `stride_bytes`.
- Accepted fix: browser capability metadata now marks browser-GPU validation as required when the effective backend is `BrowserWebGpu`.
- Accepted fix: the track validator now scans the demo README, track spec, handoff, test matrix, and WebGPU comparison doc for premature runtime/performance wording rather than only one comparison document.
- Accepted clarification: the central smoke/parity gates are static/scaffold gates only; real browser execution and runtime Wasm/WebGPU parity remain blocked.
- Validation: `pwsh -NoProfile -File conductor\tracks\33-webgpu-compute-browser\validate-track33.ps1` passed after the fixes.
- Runtime validation: `pwsh -NoProfile -File conductor\tracks\33-webgpu-compute-browser\validate-track33.ps1 -RunRuntimeTests` now passes by running `cargo test` through `stable-x86_64-pc-windows-gnu`. This proves the dependency-free WebGPU crate tests; it is still not browser WebGPU device proof.

## Review remediation -- 2026-05-18

- Accepted fix: `BufferBridge::new` now rejects `stride_bytes == 0` with a typed `BufferBridgeError::ZeroStride` before modulo validation, so malformed descriptors do not panic.
