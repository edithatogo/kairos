...
| Release artifacts are attested but the artifact tree is incomplete | 3 | 4 | 12 | RC and 1.0 require `RELEASE.txt`, `SHA256SUMS`, and `sbom.spdx.json` checks before attestation | release-agent | Attestation workflow runs against a `dist` tree missing required release files |
| Exception records are not machine-traceable | 2 | 4 | 8 | Require failing control, affected stage, compensating control, approvers, expiry, and issue/ADR reference in every exception | security-agent | Release signoff cites an exception without an owner or expiry |
