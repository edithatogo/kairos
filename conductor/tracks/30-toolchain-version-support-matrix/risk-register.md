...
|---|---:|---:|---:|---|---|---|---|
| A binding track ships with an undocumented minimum version | 2 | 4 | 8 | Matrix now records each Track 06-11 manifest floor; `toolchain-check.yml` triggers on manifest changes, and Track 30 validates the TypeScript `engines.node` floor. | toolchain-agent | Any binding enters beta without min-version declaration in `conductor/toolchain-matrix.md`. | Mitigated for current manifests; TypeScript now declares `engines.node = ">=22 <25"`. |
...
| Existing package dry-run workflow still references Go 1.24 | 2 | 3 | 6 | Package dry-run now uses the supported Go 1.25 floor, matching the Track 30 support matrix. | ci-agent + release-agent | Release packaging reintroduces Go 1.24 after the deprecation notice. | Mitigated by workflow update and validator coverage. |
