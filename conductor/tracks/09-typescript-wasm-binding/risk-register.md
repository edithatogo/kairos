...
| Node runtime support drifts from the package manifest | 2 | 3 | 6 | `package.json` declares `engines.node = ">=22 <25"` and Track 30 validates it against the support matrix | typescript-agent + toolchain-agent | CI adds or drops a production Node lane without a matching manifest and matrix update |
