...
2. Use the M1 MacBook Pro for local follow-up runs when device-specific Metal
   behavior matters.
3. Use NVIDIA NIM for Track 32-style NVIDIA library smoke checks when an
   endpoint or container is available.
4. Use Colab TPU only for the Colab notebook in `notebooks/colab_tpu_smoke.ipynb`;
...
- Colab TPU notebook: `notebooks/colab_tpu_smoke.ipynb`
