...
pub fn parse_jsonld(input: &str) -> Result<OntologyDocument, ParseError> { 
    ...
    for (index, raw_line) in input.lines().enumerate() { 
        ...
        if contains(bytes, &[114, 100, 102, 115, 58, 80, 114, 111, 112, 101, 114, 116, 121]) { 
            current_kind = 2; 
        }
        if contains(bytes, &[114, 100, 102, 115, 58, 100, 111, 109, 97, 105, 110]) { 
        ...
        if bytes.starts_with(&[125]) { 
            if let Some(id) = current_id.take() { 
                if current_kind == 1 { 
                    ...
                    let domain = current_domain.take().ok_or(ParseError { line: index + 1 })?; 
